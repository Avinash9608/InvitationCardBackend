...
exports.getAllBackups = async (req, res) => {
  try {
    // Fetch all backup wedding records from the database
    const backups = await BackupWeddingForm.find();

    // Check if any records exist
    if (!backups || backups.length === 0) {
      return res.status(404).json({ message: "No backups found in the database" });
    }
